...
public class RandomTester {
    ...
    private Logger logger = LoggerFactory.getLogger(RandomTester.class);
    ...
    byte[] getKey(long key) {
        final byte[] bytes = Convert.long2bytes(key);
        Convert.long2bytes(APHash(bytes), bytes, 0);
        return bytes;
    }
        ...
        public void run() {
            for (long i = start; i < end; i++) {
                ...
                try {
                    ...
                    if (!match(key, read)) {
                        logger.error("want {} {} get {} ", i, Convert.bytes2long(key), Convert.bytes2long(read, 0));
                        System.exit(-1);
                    }
                } catch (EngineException e) {
